...
//shows the add to watchlist option--------
	...
	function watsnew(){
		$query = $conn =query("SELECT * FROM products WHERE status = 0 ORDER BY productid DESC LIMIT 0,1") or die (mysql_error());
		while($row = mysql_fetch_array($query)){
		echo 
 ...
 //shows categories--------
	...
	function categories(){
      	$query =  mysqli_query($conn= new mysqli('localhost','root','','biddingsystemdb'),"SELECT * FROM `pcategories`") or die (mysql_error());
...
//shows latest products-----
	...
	function latest(){
		$query = mysqli_query($conn= new mysqli('localhost','root','','biddingsystemdb'),"SELECT * FROM products WHERE status = 0 ORDER BY productid DESC LIMIT 0,6") or die (mysql_error());
...
function logform(){
	...
	echo '<div class="title_box">Welcome</div>
		...
		$query = mysqli_query($conn= new mysqli('localhost','root','','biddingsystemdb'),"SELECT * FROM member WHERE memberid = '$hisid' ");
