<?php
	...
	if (isset($_POST['next2'])){
				...
				mysqli_query($conn= new mysqli('localhost','root','','biddingsystemdb'),"INSERT INTO member(lastname,firstname,gender,userid,password,email,contactno,birthdate,address,verification,memberimg) VALUES ('$lname','$fname','$gender','$userid','$password','$email','$contactno','$birthdate','$cusaddress','no','default.jpg')");
				$query = mysqli_query($conn= new mysqli('localhost','root','','biddingsystemdb'),"SELECT * FROM member WHERE userid = '$userid'") or die (mysql_error());
					$row = $query->fetch_array();
					$id= $row['memberid'];
					mysqli_query($conn= new mysqli('localhost','root','','biddingsystemdb'),"INSERT into secretquestions(memberid,question,answer) VALUES ('$id','$secques','$secanswer')");
				$_SESSION['ID']= $id;
	...
	if(isset($_POST['login'])){
				if(isset($_POST['user'])) {
					if(isset($_POST['pass'])){			
						$username = $_POST['user'];	
						$pass = $_POST['pass'];
						$query = mysqli_query($conn= new mysqli('localhost','root','','biddingsystemdb'),"SELECT * FROM member WHERE userid = '$username' AND  password = '$pass'") or die (mysql_error());
